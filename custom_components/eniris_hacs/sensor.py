...
CONCEPTUAL_MEASUREMENT_SENSORS = {
    DEVICE_TYPE_HYBRID_INVERTER: [
        ...
        (
            "summed_child_battery_charging_power",
            "Summed Battery Charging Power",
            UnitOfPower.WATT,
            SensorDeviceClass.POWER,
            SensorStateClass.MEASUREMENT,
        ...
        (
            "summed_child_battery_discharging_power",
            "Summed Battery Discharging Power",
            UnitOfPower.WATT,
            SensorDeviceClass.POWER,
            SensorStateClass.MEASUREMENT,
...
class EnirisHacsSensor(EnirisHacsEntity, SensorEntity):
    ...
    def __init__(
        ...
        current_device_props = (child_device_data or primary_device_data).get(
            "properties", {}
        )
        device_name_prefix = current_device_props.get(
            "name", current_device_props.get("nodeId", "Unknown Device")
        )

        # Construct a unique key part based on retention policy and data type
        key_suffix = (
            f"_{self._retention_policy_tag}_{self._data_type_tag}"
            if self._retention_policy_tag
            else ""
        )
        ...
        self._attr_unique_id = (
            f"{current_device_props.get('nodeId')}_{self._value_key_for_unique_id}"
        )
    ...
    def _get_sensor_value_from_latest_data(
        self,
        ...
        if not field_data or not isinstance(field_data, dict):
            ...
            if (
                rp_tag is None and data_type == "latest"
            ):  # Assuming direct value if no RP tag
                return field_data
            return None

        if rp_tag:  # rp_one_m
            return field_data.get(f"{rp_tag}_{data_type}")
        # Should not happen if rp_tag is expected, but as a fallback for non-rp specific latest/sum
        return field_data.get(data_type)

    def _update_internal_state(self) -> None:
        """Update the internal state of the sensor from the current device data."""
        current_device_data_for_sensor = (
            self._get_current_device_data_from_coordinator()
        )
        ...
        if self._is_info_sensor:
            new_value = get_value_from_info(
                current_device_data_for_sensor, self._base_value_key
            )
        ...
        new_sensor_value: Any = (
            None  # Temporary variable to hold the potential new value
        )
        ...
        # Charging/Discharging Power
            ...
            total_power = 0
            power_data_found = (
                False  # Did we find any relevant power data to make a calculation?
            )
            ...
            if node_type_of_current_device == DEVICE_TYPE_HYBRID_INVERTER:
                ...
                for child_block in current_device_data_for_sensor.get(
                    "_processed_children", []
                ):
                    if (
                        child_block.get("properties", {}).get("nodeType")
                        == DEVICE_TYPE_BATTERY
                    ):
                        ...
                        value = self._get_sensor_value_from_latest_data(
                            ...
                            "rp_one_m",
                            "latest",
                        )  # Using rp_one_m for this summary
                        if value is not None:
                            total_power += value
                ...
                value = self._get_sensor_value_from_latest_data(
                    latest_data_source, "actualPowerTot_W", "rp_one_m", "latest"
                )  # Using rp_one_m for this summary
                if value is not None:
                    total_power = value
                    power_data_found = True
            ...
            if power_data_found:
                if self._base_value_key == "charging_power":
                    new_sensor_value = max(0, total_power)
            ...
            if node_type_of_current_device == DEVICE_TYPE_HYBRID_INVERTER:
                for child_block in current_device_data_for_sensor.get(
                    ...
                    if (
                        ...
                        battery_latest_data = child_block.get("_latest_data", {})
                        soc_value_from_api = self._get_sensor_value_from_latest_data(
                            battery_latest_data,
                            "stateOfCharge_frac",
                            "rp_one_m",
                            "latest",
                ...
                soc_value_from_api = self._get_sensor_value_from_latest_data(
                    latest_data_source,
                    "stateOfCharge_frac",
                    self._retention_policy_tag or "rp_one_m",
                    "latest",
                )
        ...
        # Summed real-time charging power of child batteries for a Hybrid Inverter
            ...
            self._base_value_key == "summed_child_battery_charging_power"
            and current_properties.get("nodeType") == DEVICE_TYPE_HYBRID_INVERTER
        ):
            ...
            for child_block in current_device_data_for_sensor.get(
                ...
                if (
                    ...
                    battery_latest_data = child_block.get("_latest_data", {})
                    power_value = self._get_sensor_value_from_latest_data(
                        battery_latest_data,
                        "actualPowerTot_W",
                        self._retention_policy_tag,
                        "latest",
            ...
            if found_any_battery_data:
                new_sensor_value = max(0, net_summed_child_battery_power)
                value_is_available_for_update = True

        # Summed real-time discharging power of child batteries for a Hybrid Inverter
        elif (
            self._base_value_key == "summed_child_battery_discharging_power"
            and current_properties.get("nodeType") == DEVICE_TYPE_HYBRID_INVERTER
        ):
            net_summed_child_battery_power = 0
            found_any_battery_data = False
            ...
            if found_any_battery_data:
                new_sensor_value = (
                    abs(net_summed_child_battery_power)
                    if net_summed_child_battery_power < 0
                    else 0
                )
                value_is_available_for_update = True
        ...
        # --- Generic Handling for telemetry data based on retention_policy_tag and data_type_tag ---
            ...
            self._retention_policy_tag and self._data_type_tag
        ):  # Ensure it's a telemetry sensor
            value = self._get_sensor_value_from_latest_data(
                latest_data_source,
